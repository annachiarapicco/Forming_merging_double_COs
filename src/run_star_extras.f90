...
! ***********************************************************************
      ...
      subroutine my_kap_get( &
         ...
         real(dp), intent(in) :: X, Z, XC, XN, XO, XNe !,Zbase    
         ...
         real(dp) :: alpha, Zmultiplier, Z2, Zbase2
         ...
         if (s% star_age < 1d6) then
            Zmultiplier = 1d-3
         else if (s% star_age < 2d6) then
            alpha = (s% star_age-1d6)/(2d6-1d6)
            Zmultiplier = alpha*0d0 + (1-alpha)*(-3d0)
            Zmultiplier = 10**(Zmultiplier)
         else
            Zmultiplier = 1
         end if
         if (k==1) write(*,*) "check Zmultiplier", Zmultiplier
         if (k==1) write(*,*) "check AGE (yr)", s% star_age
         ! Zbase2 = s% kap_rq% Zbase
         Z2 = Z*Zmultiplier
         s% kap_rq% Zbase = Z2
      ...
      subroutine data_for_extra_history_columns(id, n, names, vals, ierr)
         ...
         names(1) = "Gamma_e"
         vals(1) = 0.2*(1+s% X(1))*s% L(1)/(4*pi*standard_cgrav*clight*s% m(1))
         write(*,*) "Gamma_e=", vals(1)
         s% xtra(1) = vals(1)
      ...
      subroutine extras_after_evolve(id, ierr)
         ...
         write(*,*) "He ZAMS values:", s% star_mass, s% Teff, s% L_surf, s% r(1)/Rsun, s% xtra(1)
